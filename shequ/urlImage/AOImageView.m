...
- (id)initWithImageName:(NSString *)imageName title:(NSString *)titleStr x:(float)xPoint y:(float)yPoint height:(float)height
{
    ...
    if (self) {
        ...
        //因为它的容器的位置是屏幕的左上角，而我们的容器的位置是在tag是321的imageView那里，不需要有一个64的偏移量
        UrlImageButton *imageView = [[UrlImageButton alloc] initWithFrame:CGRectMake(3, 0, 314, height)];
        //设置网络图片路径
        ...
        //设置背景条
        UIView *titleBack = [[UIView alloc]initWithFrame:CGRectMake(3, self.frame.size.height - 25, 314, 25)];
        ...
        //设置标题文字
        UILabel *titleLabel = [[UILabel alloc]initWithFrame:CGRectMake(3, self.frame.size.height-25, 314, 25)];
